...
class Export(object):
    # True/False for allowed (thumb, art, logo, squareArt) image export
    MEDIA_TYPES = {
        'movie': (True, True, True, True),
        'show': (True, True, True, True),
        'season': (True, True, True, True),
        'episode': (False, False, True, True),
        'artist': (True, True, True, True),
        'album': (True, True, True, True),
        'track': (False, False, False, False),
        'photoalbum': (True, True, True, True),
        'photo': (False, False, False, False),
    ...
    def __init__(self, section_id=None, user_id=None, rating_key=None, file_format='csv',
                 ...
                 thumb_level=0, art_level=0, logo_level=0, squareArt_level=0,
    ...
    def return_attrs(self, media_type, flatten=False):
        ...
        def movie_attrs():
            _movie_attrs = {
                ...
                'squareArtFile': lambda o: self.get_image(o, 'squareArt'),
                'squareArtProvider': lambda o: self.get_image_provider(o, 'squareArt'),
                'studio': None,
                'summary': None,
                'tagline': None,
                'theme': None,
    ...
    def export(self):
        ...
        elif self.squareArt_level not in self.IMAGE_LEVELS:
        ...
        if self.rating_key:
            logger.debug(
                ...
                "metadata_level %d, media_info_level %d, thumb_level %s, art_level %s, logo_level %s, squareArt_level %s, "
                "file_format %s",
                self.rating_key, self.metadata_level, self.media_info_level,
                self.thumb_level, self.art_level, self.logo_level, self.squareArt_level, self.file_format)
            ...
            logger.debug(
                ...
                self.thumb_level, self.art_level, self.logo_level, self.squareArt_level, self.export_type, self.file_format)
    ...
    def _get_all_metadata_attrs(self, media_type):
        exclude_attrs = ('locations', 'media', 'artFile', 'thumbFile', 'logoFile', 'squareArtFile')
    ...
    def get_image(self, item, image):
        ...
        export_image = True
        if self.thumb_level == 1 or self.art_level == 1 or self.logo_level == 1 or self.squareArt_level == 1:
            selected = self._get_selected_image(item, image)
            export_image = selected and selected.ratingKey.startswith('upload://')
        elif self.thumb_level == 2 or self.art_level == 2 or self.logo_level == 2 or self.squareArt_level == 2:
            export_image = any(field.locked and field.name == image
                               for field in item.fields)
        elif self.thumb_level == 9 or self.art_level == 9 or self.logo_level == 9 or self.squareArt_level == 9:
            export_image = True
        ...
        filename = helpers.clean_filename('{} [{}].{}.{}'.format(item_title, rating_key, image, ext))
        filepath = os.path.join(dirpath, filename)
...
def get_export(export_id):
    db = database.MonitorDatabase()
    result = db.select_single("SELECT timestamp, title, file_format, thumb_level, art_level, logo_level, squareArt_level, "
...
def build_export_docs():
    ...
    for media_type, (thumb, art, logo, squareArt) in export.MEDIA_TYPES.items():
        ...
        for attr, level in sorted(metadata_levels_map.items(), key=helpers.sort_attrs):
            if thumb and attr == 'thumbFile' or art and attr == 'artFile' or logo and attr == 'logoFile' or squareArt and attr == 'squareArtFile':
